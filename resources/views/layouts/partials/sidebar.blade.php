...
{{-- ── Mobile topbar ── --}}
<div class="sm:hidden fixed top-0 left-0 right-0 h-topbar bg-sidebar z-[300] flex items-center justify-between px-4 border-b border-white/[0.06]">
    <a href="{{ route(config('amana-shared.home_route')) }}" class="flex items-center gap-2.5 no-underline">
        ...
        <span class="font-heading text-[15px] font-semibold text-white">{{ config('amana-shared.branding.app_name') }}</span>
    </a>
...
<aside id="mainSidebar"
    ...
    <div class="px-5 py-[22px] pb-[18px] border-b border-white/[0.06] relative z-10">
        <a href="{{ route(config('amana-shared.home_route')) }}" class="flex items-center gap-[11px] no-underline">
            <span class="w-[38px] h-[38px] rounded-full overflow-hidden flex-shrink-0 shadow-[0_4px_12px_rgba(0,0,0,0.3)] inline-block">
                <img src="{{ asset('favicon-96x96.png') }}" alt="AMANA" class="w-full h-full object-cover scale-90">
            </span>
            <div class="flex flex-col">
                <span class="font-heading text-[16px] font-semibold text-white leading-none tracking-[0.2px]">AMANA</span>
                <span class="text-[10px] text-white/35 tracking-widest uppercase font-medium mt-0.5">{{ config('amana-shared.branding.tagline_short', '') }}</span>
            </div>
        </a>
    </div>
    ...
    <div class="px-3.5 py-3.5 border-t border-white/[0.06]">
        <div class="flex items-center gap-2.5 px-2.5 py-2 rounded-sm bg-white/[0.05]">
            <div class="w-8 h-8 bg-accent rounded-full flex items-center justify-center text-xs text-white font-bold flex-shrink-0">
